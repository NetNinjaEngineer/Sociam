...
public sealed class FileService : IFileService
{
    ...
    public async Task<Result<string>> CloudinaryUploadSingleFileAsync(IFormFile file)
    {
        var isValid = ValidateFile(file);
        if (!isValid)
            return Result<string>.Failure(HttpStatusCode.BadRequest, "Invalid file format (Image/Video)");
        ...
        switch (fileType)
        {
            ...
            default:
                return Result<string>.Failure(HttpStatusCode.BadRequest, "Unsupported file type");
        }
    ...
    public Task<Result<string>> CloudinaryUploadMultipleFilesAsync(IFormFileCollection files)
    {
