    ...
    IValidator<EditPostCommand> editPostValidator) : IPostsService
{
    ...
    public async Task<Result<bool>> AddReactionAsync(AddReactionCommand command)
    {
        ...
        var isReacted = await unitOfWork.PostRepository.IsUserHasReactedToPostAsync(command.PostId, currentUser.Id);

        if (isReacted)
            return Result<bool>.Failure(HttpStatusCode.BadRequest, "You have already reacted to this post");
